...
Given an NCT ID with posted ClinicalTrials.gov results and a user-supplied protocol PDF, it
emits
...
an explicit, cited (literature backed) causal DAG**. Alongside the CSVs it produces a validated
...
The skill runs as a  **eight-step agent workflow** 
 
![Agent checklist: Verify environment, then Steps 1–8 from intake to HTML render](assets/todos.png)
